...
STEPS = [
    ...
    },
    {
        ...
        "forms": ["유산/사산 휴가 신청서"],
        "warn": [
            "현재 제공된 정보 기준으로 플로우 양식 탭에는 유산·사산 휴가 신청서가 포함되어 있습니다.",
...
left_col, main_col = st.columns([0.82, 3.18], gap="medium")
...
with left_col:
    ...
    for idx, s in enumerate(STEPS):
        prefix = "● " if idx == st.session_state.active_step else ""
        label = f"{prefix}STEP {s['id']}  {s['short']}"
        if st.button(label, key=f"step_btn_{idx}", use_container_width=True):
            st.session_state.active_step = idx
            st.rerun()
    st.markdown('<div style="height:0.6rem"></div>', unsafe_allow_html=True)
    st.markdown('<div class="card-title">양식 안내</div>', unsafe_allow_html=True)
    st.markdown(
        f"<div class="helper-box"><b>경로</b><br>{COMMON_FORM_GUIDE['location']}<br><br><b>양식명</b><br>{COMMON_FORM_GUIDE['form_name']}</div>",
        unsafe_allow_html=True,
    )
...
with main_col:
    st.markdown('<div class="stage-detail-card">', unsafe_allow_html=True)
    st.markdown(
        f"""
    ...
    st.markdown('<div class="section-box" style="margin-bottom:14px;">', unsafe_allow_html=True)
          ...
          <div class="quick-card">
            ...
            <div class="status-grid">
              <div class="status-box">
                <div class="status-label">지금 할 것</div>
                <div class="status-value">STEP {step['id']}<br>{step['short']} 응대</div>
              </div>
              <div class="status-box">
                <div class="status-label">확인할 대상</div>
                <div class="status-value">{step['target']}</div>
              </div>
              <div class="status-box">
                <div class="status-label">다음 행동</div>
    ...
    with prev_col:
        if st.session_state.active_step > 0:
            if st.button("← 이전 단계", use_container_width=True):
                ...
                st.rerun()
    with center_col:
        st.markdown(f"<div style='text-align:center; color:#708191; font-size:0.86rem; padding-top:0.7rem;'>STEP {step['id']} / {len(STEPS)}</div>", unsafe_allow_html=True)
    with next_col:
